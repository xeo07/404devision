...
const { getGoodById, updateGood } = useGoods()
...
watch(
  ...
  (newValue) => {
    ...
    form.price = String(newValue.price)
    form.image = newValue.image || placeholderImage
    form.shortDescription = newValue.shortDescription
    form.description = newValue.description
    form.installationTime = newValue.installationTime
    form.warranty = newValue.warranty
    form.brand = newValue.specs.brand
    form.type = newValue.specs.type
    form.compatibility = newValue.specs.compatibility
    form.result = newValue.specs.result
  },
  { immediate: true }
)
...
function submitForm() {
  ...
  updateGood(route.params.id, { ...form })
...
<template>
  <section v-if="good" class="card edit-box">
    <h1 class="section-title">Редактирование услуги</h1>
    <p class="section-text">Все данные выбранной услуги уже заполнены в форме.</p>
    ...
    <form class="form-grid" @submit.prevent="submitForm">
      ...
      <div>
        <button class="button-main" type="submit">Сохранить услугу</button>
      </div>
    </form>
  </section>

  <section v-else class="card edit-box">
    <h1 class="section-title">Услуга не найдена</h1>
    <p class="section-text">Редактирование недоступно, потому что услуги по такому адресу нет.</p>
  </section>

  <ModalWindow
    v-if="isModalOpen"
    title="Редактирование"
    text="Данные услуги сохранены."
    @close="closeModal"
  />
</template>
